...
def graficar_tiempos_ajuste(params, resultados, coeficientes):
    ...
    ax.set_xlabel('Cantidad de palabras en el diccionario (p)')
    ax.set_ylabel('Tiempo de ejecución (s)')
    ax.legend()
    plt.show()

def calcular_error(x, resultados, coeficientes):
    errors = [np.abs(funcion(np.array([LARGO_CADENA, p]), coeficientes[0], coeficientes[1]) - resultados[(LARGO_CADENA, p)]) for p in x]
...
def parsear_argumentos():
    ...
    parser.add_argument('--min', type=int, default=100, help='Tamaño mínimo de cantidad de palabras en la cadena desencriptada (por defecto 100)')
    parser.add_argument('--max', type=int, default=1_000, help='Tamaño máximo de cantidad de palabras en la cadena desencriptada (por defecto 1.000)')
...
def main():
    ...
    # Representan la cantidad de palabras en el diccionario (p)
    x = [LARGO_CADENA] # Fijamos la cantidad de palabras en la cadena desencriptada
    y = np.linspace(valor_minimo, valor_maximo, cantidad_puntos).astype(int) # Variamos la cantidad de palabras
    print(f"Cantidad de palabras (cada elemento i es la cantidad de elementos de la invocacion i): {y}\n")
    ...
    # Tecnica de cuadrados minimos: Ajusta los parametros c1 y c2 de la funcion a los datos de la medicion
    ...
    # Calcular el error
    errors, sse = calcular_error(y, resultados, coeficientes)
    print(f"Error cuadrático total (SSE): {sse:.10f}")

    # Graficar el error
    graficar_error(y, errors, sse)
